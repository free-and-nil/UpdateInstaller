...
// Sets text mode.
procedure TextMode({%H-}Mode: Word);
...
begin
  ...
  begin
    Result := #$FF;
    GetConsoleMode(StdIn, KeyMode{%H-});
    SetConsoleMode(StdIn, 0);
    repeat
      ReadConsoleInput(StdIn, InputRec{%H-}, 1, NumRead{%H-});
...
begin
  Result := False;
  GetNumberOfConsoleInputEvents(StdIn, NumEvents{%H-});
  if NumEvents = 0 then
    Exit;
  SetLength(InputRecArray{%H-}, NumEvents);
  PeekConsoleInput(StdIn, InputRecArray[0], NumEvents, NumRead{%H-});
...
begin
  LastMode := TextAttr;
  TextAttr := (TextAttr and $0F) {%H-}or ((Color shl 4) and $F0);
  SetConsoleTextAttribute(StdOut, TextAttr);
end;
...
begin
  GetConsoleSCreenBufferInfo(StdOut, BufferInfo{%H-});
...
begin
  ...
  begin
    ...
    FillConsoleOutputCharacterA(StdOut, ' ', Len, StartPos, NumWritten{%H-});
...
begin
  ...
  Pos.X := GetCursorX;
  Pos.Y := GetCursorY;
  FillConsoleOutputCharacterA(StdOut, ' ', Len, Pos, NumWritten{%H-});
...
begin
  ...
  ScrollConsoleScreenBufferA(StdOut, Rect, {$IFNDEF FPC}@{$ENDIF}Rect, NewPos, Fill);
end;
...
begin
  SetConsoleTextAttribute(StdOut, TextAttr);
  WriteConsoleA(StdOut, Line, Len, NumWritten{%H-}, nil);
end;
...
begin
  SetLength(Line{%H-}, BufferSize.X); // Line only contains one line of windowed text.
  WritePtr := PAnsiChar(Line);
...
var
  Height: Smallint;
begin
  GetScreenSizes(Result{%H-}, Height{%H-});
end;
...
var
  Width: Smallint;
begin
  GetScreenSizes(Width{%H-}, Result{%H-});
end;
...
var
  Height: Smallint;
begin
  GetBufferSizes(Result{%H-}, Height{%H-});
end;
...
var
  Width: Smallint;
begin
  GetBufferSizes(Width{%H-}, Result{%H-});
end;
...
procedure InitScreenMode;
  ...
  begin
    FillChar (OSVI{%H-}, SizeOf (TOSVersionInfo), #0);

    OSVI.dwOSVersionInfoSize := SizeOf (TOSVersionInfo);

    if (GetVersionEx (OSVI)) then
      Result := OSVI.dwPlatformId
    else Result := VER_PLATFORM_WIN32_NT;
  end; { Win32Platform }
...
{$ENDIF}
  if not GetConsoleScreenBufferInfo(StdOut, BufferInfo{%H-}) then
  begin
    SetInOutRes(GetLastError);
    Exit;
  end;
