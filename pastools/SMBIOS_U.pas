...
begin
  ...
  begin
    ...
    LStart := LPatternAddr^;
    i := {%H-}NativeUInt(@ABuffer[Offset]);
    //Allow to compile on old Delphi versions,  (avoid internal compiler error)
    MaxBuffer := {$IFDEF CPUX64}NativeUInt(@ABuffer[LMax]) {$ELSE} {%H-}Integer(@ABuffer[LMax]); {$ENDIF};
    while  i <= MaxBuffer  do
    begin
      if ({%H-}PByte(i)^ = LStart) then
      begin
        Found := true;
        for j := 1 to High(APattern) do
          if ({%H-}PByte(i + j)^ <> TByteArray(LPatternAddr)[j]) then
          begin
            Found := false;
            Break;
          end;
        ...
        if Found then
        begin
          Result  := (i - {$IFDEF CPUX64}NativeUInt(@ABuffer[0]){$ELSE}{%H-}Integer(@ABuffer[0]){$ENDIF});
          Exit;
        end;
      end;
      inc(i);
...
begin
  ...
  try
    ...
    FillChar(SMBIOSEntryPoint{%H-}, SizeOf(SMBIOSEntryPoint), #0);
    repeat
      BytesRead := FileStream.Read(MagicNumber{%H-}, SizeOf(MagicNumber));
...
begin
  ...
  repeat
   {$IFDEF FPC}{$HINTS OFF}{$ENDIF}
    Move(FRawSMBIOSData.SMBIOSTableData^[Index], Header{%H-}, SizeOf(Header));
...
begin
  Result := SMBIOS_U.GetSMBiosString(@RawSMBIOSData.SMBIOSTableData^, Entry, Index);
end;
...
begin;
  ...
  begin
    ...
    FRawSMBIOSData.Length := FWbemObject{%H-}.Size;
    GetMem(FRawSMBIOSData.SMBIOSTableData, FRawSMBIOSData.Length);
    FRawSMBIOSData.DmiRevision := FWbemObject{%H-}.DmiRevision;
    FRawSMBIOSData.SMBIOSMajorVersion := FWbemObject{%H-}.SMBIOSMajorVersion;
    FRawSMBIOSData.SMBIOSMinorVersion := FWbemObject{%H-}.SMBIOSMinorVersion;
